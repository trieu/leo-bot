...
def process_chat_message(
    ...
    """
    Legacy function to instantiate and run the RAGAgent.
    This is kept for backward compatibility.
    """
    agent = RAGAgent(gemini_client=gemini_client)
    return agent.process_chat_message(
        user_id=user_id,
...
class RAGAgent:
    ...
    def _summarize_context(
        ...
        try:
            ...
            raw_output = self.client.generate_content(prompt, temperature=temperature_score)
            ...
            self._save_context_summary(user_id, touchpoint_id, summary_json["intent_label"], summary_json["intent_confidence"], summary_json)
            return summary_json
    ...
    def _save_chat_message(
        ...
        message_vector = self.embedding_model.encode(f"{role}: {message}", normalize_embeddings=True).tolist()
    ...
    def _retrieve_semantic_context(
        self, user_id: str, question: str, k: int = 20, max_length: int = MAX_CONTEXT_LENGTH
    ) -> str:
        """Retrieve semantically similar chat history entries."""
        question_vector = self.embedding_model.encode(f"user: {question}", normalize_embeddings=True).tolist()
    ...
    def _save_context_summary(
        ...
        try:
            ...
            context_embedding = self.embedding_model.encode(context_json, normalize_embeddings=True).tolist()
    ...
    def _get_context_summary(self, user_id: str, touchpoint_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            base_context = {
                "user_profile": {
                    "first_name": None, "last_name": None, "interests": [], "personality_traits": []
                },
    ...
    def _build_context_summary(self, user_id: str, touchpoint_id: str, user_message: str) -> Dict[str, Any]:
        ...
        if summarized_context:
            ...
            if updated_at:
                ...
                if (now_utc - updated_at) < timedelta(minutes=3):
                    needs_refresh = False

        if needs_refresh:
            logger.info("🔄 Refreshing summarized context...")
            retrieved_context_str = self._retrieve_semantic_context(user_id, user_message)
    ...
    def process_chat_message(
        ...
        """
        try:
            self._save_chat_message(user_id, "user", user_message, persona_id, touchpoint_id, keywords)

            summarized_context = self._build_context_summary(user_id, touchpoint_id, user_message)

            prompt = self._build_prompt(user_message, summarized_context, target_language)
            logger.info(f"🧠 Final Prompt:\n{prompt}")

            answer = self.client.generate_content(prompt, temperature=temperature_score)
            if not answer:
                return "⚠️ I couldn't find enough information to answer that confidently."

            self._save_chat_message(user_id, "bot", answer, persona_id, touchpoint_id)
